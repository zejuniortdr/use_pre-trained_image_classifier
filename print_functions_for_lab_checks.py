...
def check_command_line_arguments(in_arg):
    ...
    if in_arg is None:
        print(
            "* Doesn't Check the Command Line Arguments because 'get_input_args' "
            "hasn't been defined."
        )
    else:
...
def check_classifying_labels_as_dogs(results_dic):
    ...
    if results_dic is None:
        print(
            "* Doesn't Check the Results Dictionary because 'adjust_results4_isadog' "
            "hasn't been defined."
        )
...
def check_calculating_results(results_dic, results_stats_dic):
    ...
    if results_stats_dic is None:
        print(
            "* Doesn't Check the Results Dictionary because 'calculates_results_stats' "
            "hasn't been defined."
        )
    else:
        ...
        try:
            pct_corr_notdog = 100.0 * n_class_cnotd / n_pet_notd
        except ZeroDivisionError:
            pct_corr_notdog = 0.0

        try:
            pct_corr_breed = 100.0 * n_match_breed / n_pet_dog
        except ZeroDivisionError:
            pct_corr_breed = 0.0
